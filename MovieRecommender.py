...
from textblobs import TextBlob
...
def recommend_movies(genre=None, mood=None, rating=None, top_n=5):
    ...
    filtered_df = filtered_df.sampe(frac=1).reset_index(drop=True)
    ...
    for idx, row in filtered_df.iterrows():
        ...
        if (mood and ((TextBlob(mood).sentiment.polarity < 0 and polarity > 0) or polarity >= 0)) or not found:
...
def display_recommendations(recs, name):
    ...
    for idxx, (title, polarity) in enumerate(recs, 1):
...
def handle_ai(name):
    ...
    print(Fore.GREEN + "Avaliable Genres: ", end = "")
